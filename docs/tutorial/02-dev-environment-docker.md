...
- Three compose configurations: full containers (dev default), services-only, and production
...
| `compose.override.yaml` | Dev: adds web + worker with volume mounts | Auto-loaded by `docker compose up` |
...
### Why volume mount `.:/app`?

This mounts your local project directory into the container. When you edit a file on your host, the change is immediately visible inside the container. Combined with `runserver`'s auto-reload, this gives you the same edit-save-refresh workflow as running Django locally.
...
2. Start everything:
   ...
   This loads `compose.yaml` + `compose.override.yaml` automatically.
...
- [ ] `docker compose up` starts db, web, and worker without errors
...
- [ ] `docker compose up` again reuses the database (data persists via pgdata volume)
